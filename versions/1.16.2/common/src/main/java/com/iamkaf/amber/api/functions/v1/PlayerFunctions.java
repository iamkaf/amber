...
public final class PlayerFunctions {
    ...
    public static void sendTitle(Player player, @Nullable Component title, @Nullable Component subtitle,
        ...
        if (player instanceof ServerPlayer) {
            ServerPlayer serverPlayer = (ServerPlayer) player;
            ClientboundSetTitlesPacket timesPacket = new ClientboundSetTitlesPacket(fadeIn, stay, fadeOut);
            serverPlayer.connection.send(timesPacket);

            if (title != null) {
                ClientboundSetTitlesPacket titlePacket = new ClientboundSetTitlesPacket(ClientboundSetTitlesPacket.Type.TITLE, title);
                serverPlayer.connection.send(titlePacket);
            }

            if (subtitle != null) {
                ClientboundSetTitlesPacket subtitlePacket = new ClientboundSetTitlesPacket(ClientboundSetTitlesPacket.Type.SUBTITLE, subtitle);
    ...
    public static void clearTitle(Player player) {
        if (player instanceof ServerPlayer) {
            ServerPlayer serverPlayer = (ServerPlayer) player;
            serverPlayer.connection.send(new ClientboundSetTitlesPacket(ClientboundSetTitlesPacket.Type.TITLE, TextComponent.EMPTY));
            serverPlayer.connection.send(new ClientboundSetTitlesPacket(ClientboundSetTitlesPacket.Type.SUBTITLE, TextComponent.EMPTY));
            serverPlayer.connection.send(new ClientboundSetTitlesPacket(0, 0, 0));
            serverPlayer.connection.send(new ClientboundSetTitlesPacket(ClientboundSetTitlesPacket.Type.CLEAR, null));
        }
    }
