...
public class NodeSorting {
    ...
    public static <N extends SortableNode<N>> boolean sort(List<N> sortedNodes, String elementDescription, Comparator<N> comparator) {
        ...
        PriorityQueue<NodeScc<N>> pq = new PriorityQueue<>(new Comparator<NodeScc<N>>() {
            @Override
            public int compare(NodeScc<N> first, NodeScc<N> second) {
                return comparator.compare(first.nodes.get(0), second.nodes.get(0));
            }
        });
        sortedNodes.clear();
