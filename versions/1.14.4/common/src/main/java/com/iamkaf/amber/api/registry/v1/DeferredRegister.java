 ...
 * }</pre>
 *
 * <p>The registry parameter can be used to set the id on any
...
public class DeferredRegister<T> implements Iterable<RegistrySupplier<T>> {
    ...
    private final Registry<T> key;
    private final List<Entry<T>> entries = new ArrayList<>();
    private final List<RegistrySupplier<T>> entryView = Collections.unmodifiableList(entries);
    private boolean registered = false;
    private final String modId;

    private DeferredRegister(Supplier<RegistrarManager> managerSupplier, Registry<T> key, String modId) {
     ...
     */
    public static <T> DeferredRegister<T> create(String modId, Registry<T> key) {
    ...
    /**
     * Registers a supplier that receives the entry identifier. This is useful for
     * automatically setting the id on {@code Properties} objects.
     */
    public <R extends T> RegistrySupplier<R> register(String id, Function<ResourceLocation, ? extends R> supplier) {
        if (modId == null) {
            throw new NullPointerException("DeferredRegister created without mod id");
        }
        return register(new ResourceLocation(modId, id), supplier);
    ...
    /**
     * Registers a supplier that receives the entry identifier.
     *
     * @param id       the full identifier of the entry
     * @param supplier a factory taking the entry identifier
     */
    public <R extends T> RegistrySupplier<R> register(ResourceLocation id, Function<ResourceLocation, ? extends R> supplier) {
        return register(id, (Supplier<? extends R>) () -> supplier.apply(id));
    }
